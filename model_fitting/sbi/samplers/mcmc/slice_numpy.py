...
class SliceSamplerVectorized:
    ...
    def run(self, num_samples: int) -> np.ndarray:
        ...
        log = False
        logfn = 'sbi_log.txt'
        with open(logfn, 'w') as sbilog:
            if log: sbilog.write('max width={}\n'.format(self.max_width))
            ...
            for c in range(self.num_chains):
                ...
                # large
                #widths = np.array([0.03399999856948852, 0.01399999976158142, 0.0020000000298023225, 0.012000000476837159, 0.0020000000298023225, 0.02, 0.008999999761581421, 0.002199999988079071, 120.0, 0.005999999642372131, 12.0, 0.002800000011920929, 0.035, 0.025, 0.048000001907348634])
                # medium
                #widths = np.array([0.01699999928474426, 0.00699999988079071, 0.0010000000149011613, 0.006000000238418579, 0.0010000000149011613, 0.01, 0.0044999998807907105, 0.0010999999940395355, 60.0, 0.0029999998211860655, 6.0, 0.0014000000059604645, 0.0175, 0.0125, 0.024000000953674317])
                # small
                #widths = np.array([0.004249999821186065, 0.0017499999701976775, 0.0002500000037252903, 0.0015000000596046448, 0.0002500000037252903, 0.0025, 0.0011249999701976776, 0.0002749999985098839, 15.0, 0.0007499999552965164, 1.5, 0.00035000000149011613, 0.004375, 0.003125, 0.006000000238418579])

                #widths = [0.0009, 0.0036, 0.002025]
                widths = np.array([0.01, 0.01, 0.01])
                self.state[c]["width"] = widths
            ...
            while num_chains_finished != self.num_chains:
                ...
                if sc['state'] == 'BEGIN':
                    if log: sbilog.write('parameter {}: init val cxi={}\nf(cxi)={}\n'.format(labels[sc['order'][sc['i']]],sc['cxi'],log_probs[c]))
                elif sc['state'] == 'LOWER':
                    if log: sbilog.write('f(lx)={}\n'.format(log_probs[c]))
                elif sc['state'] == 'UPPER':
                    if log: sbilog.write('f(ux)={}\n'.format(log_probs[c]))
                else:
                    if log: sbilog.write('f(xi)={}\n'.format(log_probs[c]))
                ...
                for c in range(self.num_chains):
                    ...
                    if sc["state"] == "BEGIN":
                        # position the bracket randomly around the current sample
                        # Draw some random position on the pdf below log_probs[c] @ x?
                        # But why is the adjustment so small compared to log_prob?
                        sc["logu"] = log_probs[c] + np.log(1.0 - self.rng.rand())
                        # Why is only the lower bound shift multiplied by rand?
                        sc["lx"] = sc["cxi"] - sc["wi"] * self.rng.rand()
                        sc["ux"] = sc["lx"] + sc["wi"]
                        if log: sbilog.write('initial slice bounds: ({},{}) | wi={}\n'.format(sc['lx'],sc['ux'],sc['wi']))
                        sc["next_param"] = np.concatenate(
                            [
                                sc["x"][: sc["order"][sc["i"]]],
                                [sc["lx"]],
                        ...
                        if outside_lower:
                            ...
                            if my_verbose: print('outside lower, lx={}'.format(sc['lx']))    
                            if log: sbilog.write('lx moved to {}\n'.format(sc['lx']))
                            ...
                            sc["state"] = "UPPER"
                            if log: sbilog.write('log(f(lx)={}) < log(y)={}\nmoving to upper bound\n'.format(log_probs[c], sc['logu']))
                        ...
                        if outside_upper:
                            ...
                            if log: sbilog.write('ux moved to {}\n'.format(sc['ux']))
                        else:
                            # sample uniformly from bracket
                            ...
                            if log: sbilog.write('log(f(ux)={}) < log(y)={}\nsample param at {}\n'.format(log_probs[c], sc['logu'], sc['xi']))
                        ...
                        if rejected:
                            if sc["xi"] < sc["cxi"]:
                                sc["lx"] = sc["xi"]
                                if log: sbilog.write('slice sample {} rejected, set to new lx\n'.format(sc['xi']))
                            else:
                                sc["ux"] = sc["xi"]
                                if log: sbilog.write('slice sample {} rejected, set to new ux\n'.format(sc['xi']))
                            sc["xi"] = (sc["ux"] - sc["lx"]) * self.rng.rand() + sc["lx"]
                            sc["next_param"] = np.concatenate(
                                [
                                    sc["x"][: sc["order"][sc["i"]]],
                            ...
                            if log: sbilog.write('sampling {}={} from slice [{},{}]\n'.format(labels[sc['order'][sc['i']]], sc['xi'], sc['lx'], sc['ux']))
                            if sc["t"] < num_samples:
                                #if log: sbilog.write('move to next parameter\n')
                                if log: sbilog.write('\n')
                                sc["state"] = "BEGIN"
                                ...
                                #if sc["t"] <= (self.tuning):
                                if sc["t"] < (self.tuning):
                                    if log: sbilog.write('tuning bracket width\n')
                                    i = sc["order"][sc["i"]]
                                    sc["width"][i] += (
