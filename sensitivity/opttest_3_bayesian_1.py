...
class NN(Model):
    ...
    def forward(self, X, num_samples=10):
        outputs = [self._forward(X) for _ in range(num_samples)]  
        pred_mean = torch.mean(torch.stack([o[:, 0] for o in outputs]), dim=0).unsqueeze(1)
        pred_std = torch.std(torch.stack([o[:, 0] for o in outputs]), dim=0).unsqueeze(1)
...
def sample_elbo(model, x, y, criterion=None, sample_nbr=3, complexity_cost_weight=1e-6):
    ...
    for _ in range(sample_nbr):
        ...
        if criterion != None:
            ...
            total_nll += nll_with_min_variance_penalty(y, mean, torch.square(std))
...
for epoch in range(NUM_EPOCHS):
    ...
    loss = sample_elbo(model, train_x, train_y, sample_nbr=1)
    loss.backward()
    optimizer.step()
