    ...
    for constraint_i, baseline_area in enumerate(baseline_areas):
        ...
        for metric in metrics:
            if my_rank == 0:
                ...
                phase_data = {}
                phase_data["fri_max"] = np.ones((len(ncell_vec), fri_max_samples)) * np.nan
                phase_data["<fri_init>"] = np.ones((len(ncell_vec), fri_max_samples)) * np.nan
                phase_data["<metric>"] = np.ones((len(ncell_vec), fri_max_samples)) * np.nan
                # Add a matrix for computing excess resources; only do this once
                if metric == metrics[0]:
                    phase_data["<xs>"] = np.ones((len(ncell_vec), fri_max_samples)) * np.nan
            ...
            for ncell_i, ncell in enumerate(tqdm(ncell_vec, disable=(not progress))):
                ...
                # First find fri_max where slice with lowest initial fri satisfies C
                # This gives us a lower bound on fri_max which can satisfy C
                # Then find fri_max where slice with highest initial fri satisfies C
                # This gives us an upper bound on fri_max which can satisfy C
                fri_max_bounds = []
                fri_max = constraint / ncell #starting value for the scan
                for lb in [slice_left_min, slice_left_max]:
                    fri_slice = fri_sorted[lb:lb+ncell]
                    bound_located = False
                    while not bound_located:
                        check = np.sum(fri_max - fri_slice)
                        if check >= constraint:
                            bound_located = True
                        else:
                            fri_max += 0.1
                    fri_max_bounds.append(fri_max)
                num_samples = fri_max_samples
                fri_max_vec = np.linspace(min(fri_max_bounds), max(fri_max_bounds), num_samples)
                ...
                # Loop over different realizations of this fire alteration strategy
                for sub_sample_i, fri_max_sample_i in enumerate(tqdm(range(sub_start, sub_start+sub_samples), disable=True)):#(not progress))):
                    # Get fri_max sample, skip if outside allowed fric bounds
                    fri_max = fri_max_vec[fri_max_sample_i]
                    if (fri_max < min_fri) or (fri_max > final_max_fri):
                        continue
                    # Re-sort fri and get slice for this rank
                    fri_sorted = fri_expected[fri_argsort]
                    # Find slice which gets us closest to C for this (fri_max, ncell) pair
                    sums = scipy.signal.fftconvolve((fri_max - fri_sorted[slice_left_min:]),np.ones(ncell,dtype=int),'valid')
                    diffs = np.abs(sums - constraint)
                    slice_left = np.argmin(diffs) + slice_left_min 
                    fri_slice = fri_sorted[slice_left:slice_left + ncell]
                    if sub_sample_i < len(sub_fri_means): #Skip if computing no change scenario
                        sub_fri_means[fri_max_sample_i-sub_start] = np.mean(fri_slice)
                    # Get the per cell fric values
                    fric_slice = fri_max - fri_slice
                    '''fric should never push fri past final_max_fri, i.e. no xs resources'''
                    if np.any((fric_slice + fri_slice) > final_max_fri): print('check me', fri_sorted[slice_left])
                    ...
                    if sub_sample_i < len(sub_fri_means): #Skip if computing the no change scenario
                        ...
                        if metric == metrics[0]:
                            ...
                            if len(xsresources) > 0:
                                sub_xs_means[fri_max_sample_i-sub_start] = np.mean(xsresources)
                    ...
                    if sub_sample_i < len(sub_fri_means):
                        sub_metric_expect[fri_max_sample_i-sub_start] = metric_expect
                    ...
                    # Otherwise save no change scenario to file
                        ...
                        print(f"Not adding sample with index {fri_max_sample_i} / {fri_max_sample_i-sub_start} on rank {my_rank}, instead saving as nochange")
                ...
                if my_rank == 0:
                    # Store data collected at this ncell value
                    phase_data["fri_max"][ncell_i,:] = fri_max_vec
                    phase_data["<fri_init>"][ncell_i,:] = sampled_fri_means
                    phase_data["<metric>"][ncell_i,:] = sampled_metric_expect
                    if metric == metrics[0]:
                        phase_data["<xs>"][ncell_i,:] = sampled_xs_means
                    # Also store data on max(<metric>) at (constraint, ncell, delta_fri)
                    phase_slice = phase_data["<metric>"][ncell_i,:]
                    '''need to fix this zero issue, shouldnt be happening'''
                    phase_slice = phase_slice[phase_slice != 0]
            ...
            if my_rank == 0:
                ...
                phase_fn = data_root + "/scaledfric_phase.json"
                # Need to convert numpy arrays to lists before saving as json
                for key, obj in phase_data.items():
                    phase_data[key] = obj.tolist()
                with open(phase_fn, 'w') as handle:
                    json.dump(phase_data, handle)
